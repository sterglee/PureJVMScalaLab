...
// multiplication using openblas
 final def * (that: Array[Array[Double]]): Mat =  {
   if (isLargeMatrix==true )   // seems a large multiplication problem
    {
   new Mat(scalaSci.ParallelMult.pmul(this.v, that))
 }
     else
   new Mat(scalaSci.SerialMult.times(this.v, that))
...
// multiplication using openblas
 override final def * (that: RichDouble2DArray): Mat =  {
     if (isLargeMatrix==true )   // seems a large multiplication problem
    {
   new Mat(scalaSci.ParallelMult.pmul(this.v, that.getArray))
 }
     else
   new Mat(scalaSci.SerialMult.times(this.v, that.getArray))
